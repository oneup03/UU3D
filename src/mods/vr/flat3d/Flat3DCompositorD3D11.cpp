...
bool Flat3DCompositorD3D11::composite(ID3D11DeviceContext* context,
    ...
    if (params.hud_depth_mode == 1 && m_hud_depth_srv != nullptr && ui_srv != nullptr && m_classify_ps != nullptr) {
        ...
        if (m_hud_mask_tex[0] != nullptr && ui_tex2d != nullptr && device != nullptr) {
            ...
            if (m_hud_prev_ui != nullptr) {
                ...
                // The classify pass runs in UI-TARGET uv, but flow and the
                // user's region settings are authored in SCREEN (eye) uv. The
                // overlay reads a central crop of the UI, so map screen->UI:
                // extent *= crop; off-centre position re-centres about 0.5.
    ...
    if (params.mode == (int32_t)Flat3DOutputMode::LEIA_SR && leiasr_available()) {
        // The lens preference tracks whether we actually wove, not the
        // requested mode: a LeiaSR frame that fell back must not leave the
        // panel lensed either.
        const bool woven = weave_leiasr(context, backbuffer_rtv, out_w, out_h, params);
        set_leiasr_lens(woven);
        return woven;
    }

    // Any other output mode: hand the lens back. Idempotent, so this costs
    // nothing on the frames where we never held it.
...
void Flat3DCompositorD3D11::draw_overlays(ID3D11DeviceContext* context, ID3D11ShaderResourceView* ui_srv,
    ...
    for (int eye = 0; eye < 2; ++eye) {
        ...
        auto draw_layer = [&](int32_t layer, float shift_uv, float scale, float center_x, float center_y) {
            ...
            // Folded into the src-uv map about center 0.5. Other layers (menu,
            // cursor) are authored at eye aspect already => no crop.
            // Native Render + Upscale: game HUD (0/1) AND UEVR menu (3) draw at
            // the perceived half width into the TOP-LEFT of a full-width target —
            // sample that top-left region (anchor 0) rather than the centre.
            ...
            // UI center within the target: central (0.5), or the centre of the
            // TOP-LEFT crop region (crop/2) under Native Render + Upscale. The
            // fit-squish `scale`, scene_scale and per-eye depth shift are applied
            // about that centre, so the GUI still stays on-screen / squishes at
            // depth. (crop*0.5 collapses to 0.5 for central => unchanged.)
            ...
            // (UI-u is magnified 1/crop_x on screen), y needs *crop_y.
